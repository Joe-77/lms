...
const SignIn = () => {
  ...
  const { signInNow, loginWithGoogle } = useContext(Context);
    ...
    <div className="min-h-screen bg-lms bg-center bg-cover bg-no-repeat relative">
      ...
      <div className="relative z-50 w-11/12 min-[400px]:w-3/4 min-[500px]:w-[60%] sm:w-1/2 lg:w-1/4 mx-auto mt-28 lg:mt-36  py-5 rounded-lg">
        <form onSubmit={handleSubmit(handleFormSubmit)}>
          <h4 className="text-xl text-center text-white py-2">
            Sign in to{" "}
          ...
          <div className="w-3/4 mx-auto mt-1 flex items-center justify-between">
            <Link to="/signUp" className="text-[10px] text-zinc-500">
              don't have an account?
            </Link>
            <Link to="/reset" className="text-[10px] text-red-500">
              need help!
            </Link>
          </div>
          ...
          <button
            type="submit"
            className="w-3/4 mx-auto my-4 py-1 rounded-md text-white text-center capitalize tracking-wider text-lg bg-blue-500 hover:bg-blue-700 duration-500 block"
          >
            login
          </button>

          <div className="relative w-3/4 h-[2px] bg-gray-300 mx-auto circle"></div>

          <div className="w-3/4 mx-auto mt-5 flex gap-3">
            <div
              onClick={loginWithGoogle}
              className="w-full pl-2 sm:pl-0 duration-500 bg-pink-500 hover:bg-pink-700 text-white py-2 rounded-md flex items-center justify-center gap-1 cursor-pointer"
            >
              <span className="text-xl">
                <FaGoogle />
              </span>
              <span className="text-sm capitalize ">continue with google</span>
            </div>
          </div>
        </form>
      </div>
